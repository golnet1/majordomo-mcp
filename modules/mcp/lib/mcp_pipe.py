...
async def connect_to_server(uri, target):
    ...
    try:
        ...
        mcp_instance.debmes(f"Connecting to WebSocket server...", "mcp")
        async with websockets.connect(uri) as websocket:
            logger.info(f"[{target}] Successfully connected to WebSocket server")
            mcp_instance.debmes(f"Successfully connected to WebSocket server", "mcp")
            ...
            mcp_instance.debmes(f"Started server process: {' '.join(cmd)}", "mcp")
        ...
        mcp_instance.debmes(f"Error in WebSocket connection closed: {e}", "mcp")
        ...
        if 'process' in locals():
            ...
            mcp_instance.debmes(f"Terminating server process", "mcp")
            try:
                process.terminate()
            ...
            mcp_instance.debmes(f"Server process terminated", "mcp")
...
async def pipe_websocket_to_process(websocket, process, target):
    ...
    try:
        ...
        mcp_instance.debmes(f"Error in WebSocket to process pipe: {e}", "mcp")
        raise  # Re-throw exception to trigger reconnection
    finally:
        # Close process stdin
...
async def pipe_process_stderr_to_terminal(process, target):
    ...
    try:
        ...
        mcp_instance.debmes(f"Error in process stderr pipe: {e}", "mcp")
        raise  # Re-throw exception to trigger reconnection
...
def load_config():
    ...
    try:
        ...
        mcp_instance.debmes(f"Failed to load config {path}: {e}", "mcp")
        return {}
...
if __name__ == "__main__":
    ...
    endpoint_url = mcp_instance.mcp_endpoint
    logger.info(f"MCP_ENDPOINT: {endpoint_url}")
    if not endpoint_url:
        endpoint_url = os.environ.get('MCP_ENDPOINT')
    if not endpoint_url:
        logger.error("Please set the `MCP_ENDPOINT` environment variable")
        mcp_instance.debmes("Please set the `MCP_ENDPOINT` environment variable", "mcp")
        sys.exit(1)
    ...
    async def _main():
        if not target_arg:
            ...
            if skipped:
                ...
                mcp_instance.debmes(f"Skipping disabled servers: {', '.join(skipped)}", "mcp")
            if not enabled:
                raise RuntimeError("No enabled mcpServers found in config")
            logger.info(f"Starting servers: {', '.join(enabled)}")
            mcp_instance.debmes(f"Starting servers: {', '.join(enabled)}", "mcp")
            ...
            if os.path.exists(target_arg):
                ...
                mcp_instance.debmes("Argument must be a local Python script path. To run configured servers, run without arguments.", "mcp")
                sys.exit(1)
    ...
    try:
        ...
        mcp_instance.debmes("Program interrupted by user", "mcp")
    except Exception as e:
        logger.error(f"Program execution error: {e}")
        mcp_instance.debmes(f"Program execution error: {e}", "mcp")